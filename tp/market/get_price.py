...
from tp.market.MrktDataUtil import  getHistoricalData, get_market_price, \
    get_ticker_data
# getTickerInfo, MarketDataForTicker,
...
if __name__ == "__main__":
    # sleep_sec(10)
    t = ['ILTB', 'MSFT', 'SPHIX', 'OKTA',  'ARKK', 'AAPL', 'T']
    get_market_price_for_list(t)
    # hist_data = getHistoricalData(t)
    for i in [1,2]:
        for l in t:
            # print({l: get_market_price(l)})
            tiObj = get_ticker_data(l)
            if tiObj:
                tiObj.pretty_print_members()
            else:
                print(f"no data for {l}")
        print(f"=================={i}=====================")
            # sleep_sec(1)
